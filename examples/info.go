...
func main() {
	...
	driver.Logger().Info("+++++++++++++++++++")
	if lldp, err := driver.ListLLDPNeighbours(); err == nil {
		for _, neighbour := range lldp {
			driver.Logger().Infof("Interface: %q: %s", neighbour.LocalInterface, neighbour.RemoteHostname)
		}
	}
