...
func (fs *FSCom) ListLLDPNeighbours() ([]models.LLDPNeighbour, error) {
	output, err := fs.sendCommands("show lldp neighbors")
	if err != nil {
		return nil, err
	}

	return ParseLLDPTable(output)
}

func ParseLLDPTable(output string) ([]models.LLDPNeighbour, error) {
	var table []models.LLDPNeighbour
	...
	for _, line := range strings.Split(output, "\n") {
		...
		table = append(table, models.LLDPNeighbour{
